...
#include "SkyraGameInstance.generated.h"

class ASkyraPlayerController;
class UObject;

UCLASS(Config = Game)
class SKYRAGAME_API USkyraGameInstance : public UCommonGameInstance
{
	GENERATED_BODY()

public:

	USkyraGameInstance(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	ASkyraPlayerController* GetPrimaryPlayerController() const;
