...
#include "SkyraNumberPopComponent_MeshText.generated.h"

class USkyraDamagePopStyle;
...
class USkyraNumberPopComponent_MeshText : public USkyraNumberPopComponent
{
	GENERATED_BODY()

public:

	USkyraNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FSkyraNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo, const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FSkyraNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FSkyraNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<USkyraDamagePopStyle>> Styles;
