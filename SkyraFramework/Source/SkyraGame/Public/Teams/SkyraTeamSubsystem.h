...
#include "SkyraTeamSubsystem.generated.h"
...
public:
	UPROPERTY()
	TObjectPtr<ASkyraTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<ASkyraTeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<USkyraTeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnSkyraTeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(ASkyraTeamInfoBase* Info);
	void RemoveTeamInfo(ASkyraTeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class ESkyraTeamComparison : uint8
{
...
UCLASS()
class SKYRAGAME_API USkyraTeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	bool RegisterTeamInfo(ASkyraTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	bool UnregisterTeamInfo(ASkyraTeamInfoBase* TeamInfo);
	...
	// Blueprint wrapper for changing the team associated with this actor if possible
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category=Teams, meta=(DisplayName="Change Team For Actor"))
	bool K2_ChangeTeamForActor(AActor* ActorToChange, int32 NewTeamId);
	...
	const ASkyraPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	ESkyraTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	ESkyraTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	USkyraTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	USkyraTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	void NotifyTeamDisplayAssetModified(USkyraTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnSkyraTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FSkyraTeamTrackingInfo> TeamMap;
