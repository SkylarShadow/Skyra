...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const USkyraTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements ISkyraTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const USkyraTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const USkyraTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<ISkyraTeamAgentInterface> TeamInterfacePtr;
