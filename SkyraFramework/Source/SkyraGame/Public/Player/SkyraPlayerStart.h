...
#include "SkyraPlayerStart.generated.h"

class AController;
class UObject;

enum class ESkyraPlayerStartLocationOccupancy
{
	Empty,
	Partial,
	Full
 ...
 */
UCLASS(Config = Game)
class SKYRAGAME_API ASkyraPlayerStart : public APlayerStart
{
	GENERATED_BODY()

public:
	ASkyraPlayerStart(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	const FGameplayTagContainer& GetGameplayTags() { return StartPointTags; }

	ESkyraPlayerStartLocationOccupancy GetLocationOccupancy(AController* const ControllerPawnToFit) const;
