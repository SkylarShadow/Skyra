...
#include "SkyraReplaySubsystem.generated.h"
...
class SKYRAGAME_API USkyraReplayListEntry : public UObject
{
	GENERATED_BODY()
...
UCLASS(BlueprintType)
class SKYRAGAME_API USkyraReplayList : public UObject
{
	GENERATED_BODY()

public:
	UPROPERTY(BlueprintReadWrite, Category=Replays)
	TArray<TObjectPtr<USkyraReplayListEntry>> Results;
};

/** Subsystem to handle recording/loading replays */
UCLASS()
class SKYRAGAME_API USkyraReplaySubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

public:
	USkyraReplaySubsystem();
	...
	void PlayReplay(USkyraReplayListEntry* Replay);
