...
class USkyraReplayList;
class UObject;
struct FEnumerateStreamsResult;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FQueryReplayAsyncDelegate, USkyraReplayList*, Results);
...
private:
	UPROPERTY()
	TObjectPtr<USkyraReplayList> ResultList;
