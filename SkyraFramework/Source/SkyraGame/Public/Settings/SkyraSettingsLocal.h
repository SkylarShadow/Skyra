...
#include "SkyraSettingsLocal.generated.h"
...
USTRUCT()
struct FSkyraScalabilitySnapshot
{
	GENERATED_BODY()

	FSkyraScalabilitySnapshot();
 ...
 */
UCLASS()
class USkyraSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	static USkyraSettingsLocal* Get();
...
public:
	...
	ESkyraStatDisplayMode GetPerfStatDisplayState(ESkyraDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ESkyraDisplayablePerformanceStat Stat, ESkyraStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(USkyraSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<ESkyraDisplayablePerformanceStat, ESkyraStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FSkyraScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FSkyraScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(USkyraSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	DECLARE_EVENT_OneParam(USkyraSettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	...
	void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, USkyraLocalPlayer* LocalPlayer);
	...
	void ResetKeybindingToDefault(const FName MappingName, USkyraLocalPlayer* LocalPlayer);
	...
	void ResetKeybindingsToDefault(USkyraLocalPlayer* LocalPlayer);
