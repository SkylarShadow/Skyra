...
#include "SkyraSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class ESkyraAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * USkyraSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class USkyraSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(USkyraSettingsShared, FOnSettingChangedEvent, USkyraSettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	USkyraSettingsShared();
	...
	static USkyraSettingsShared* CreateTemporarySettings(const USkyraLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static USkyraSettingsShared* LoadOrCreateSettings(const USkyraLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, USkyraSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const USkyraLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	ESkyraAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(ESkyraAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	ESkyraAllowBackgroundAudioSetting AllowAudioInBackground = ESkyraAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	ESkyraGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(ESkyraGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	ESkyraGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(ESkyraGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	ESkyraGamepadSensitivity GamepadLookSensitivityPreset = ESkyraGamepadSensitivity::Normal;
	UPROPERTY()
	ESkyraGamepadSensitivity GamepadTargetingSensitivityPreset = ESkyraGamepadSensitivity::Normal;
