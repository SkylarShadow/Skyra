...
#include "SkyraInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class USkyraAimSensitivityData;
class UObject;
...
class USkyraSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the Skyra Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Skyra Settings Driven Dead Zone"))
class USkyraInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class ESkyraTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in Skyra Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Skyra Gamepad Sensitivity"))
class USkyraInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	ESkyraTargetingType TargetingType = ESkyraTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const USkyraAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the Skyra Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Skyra Aim Inversion Setting"))
class USkyraInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
