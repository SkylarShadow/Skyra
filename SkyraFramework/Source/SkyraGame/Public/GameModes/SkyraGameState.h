...
#include "SkyraGameState.generated.h"
...
class USkyraExperienceManagerComponent;
class UObject;
struct FFrame;
 ...
 */
UCLASS(Config = Game)
class SKYRAGAME_API ASkyraGameState : public AModularGameStateBase, public IAbilitySystemInterface
{
	GENERATED_BODY()

public:

	ASkyraGameState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	UFUNCTION(BlueprintCallable, Category = "Skyra|GameState")
	...
	UFUNCTION(NetMulticast, Unreliable, BlueprintCallable, Category = "Skyra|GameState")
	void MulticastMessageToClients(const FSkyraVerbMessage Message);

	// Send a message that all clients will be guaranteed to get
	// (use only for client notifications that cannot handle being lost)
	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "Skyra|GameState")
	void MulticastReliableMessageToClients(const FSkyraVerbMessage Message);
...
private:
	...
	UPROPERTY()
	TObjectPtr<USkyraExperienceManagerComponent> ExperienceManagerComponent;

	// The ability system component subobject for game-wide things (primarily gameplay cues)
	UPROPERTY(VisibleAnywhere, Category = "Skyra|GameState")
