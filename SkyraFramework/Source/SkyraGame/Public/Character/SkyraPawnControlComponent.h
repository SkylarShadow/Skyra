...
#include "SkyraPawnControlComponent.generated.h"
...
class SKYRAGAME_API USkyraPawnControlComponent : public UPawnComponent, public IGameFrameworkInitStateInterface
{
	GENERATED_BODY()

public:

	USkyraPawnControlComponent(const FObjectInitializer& ObjectInitializer);

	/** Returns the pawn control component if one exists on the specified actor. */
	UFUNCTION(BlueprintPure, Category = "Skyra|PawnControl")
	static USkyraPawnControlComponent* FindPawnControlComponent(const AActor* Actor) { return (Actor ? Actor->FindComponentByClass<USkyraPawnControlComponent>() : nullptr); }

	/** Overrides the camera from an active gameplay ability */
	void SetAbilityCameraMode(TSubclassOf<USkyraCameraMode> CameraMode, const FGameplayAbilitySpecHandle& OwningSpecHandle);

	/** Clears the camera override if it is set */
	void ClearAbilityCameraMode(const FGameplayAbilitySpecHandle& OwningSpecHandle);

	/** Adds mode-specific input config */
	void AddAdditionalInputConfig(const USkyraInputConfig* InputConfig);

	/** Removes a mode-specific input config if it has been added */
	void RemoveAdditionalInputConfig(const USkyraInputConfig* InputConfig);
...
protected:
	...
	UPROPERTY()
	TSubclassOf<USkyraCameraMode> AbilityCameraMode;
