...
#include "SkyraPerformanceSettings.generated.h"
...
USTRUCT()
struct FSkyraQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FSkyraPerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<ESkyraDisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class ESkyraFramePacingMode : uint8
{
...
class USkyraPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	USkyraPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const USkyraPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FSkyraQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	ESkyraFramePacingMode FramePacingMode = ESkyraFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by Skyra.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==ESkyraFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Skyra Performance Settings"))
class USkyraPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	USkyraPerformanceSettings();
...
public:
	...
	TArray<FSkyraPerformanceStatGroup> UserFacingPerformanceStats;
};
