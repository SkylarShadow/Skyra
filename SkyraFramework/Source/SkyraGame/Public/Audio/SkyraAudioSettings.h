...
#include "SkyraAudioSettings.generated.h"
...
USTRUCT()
struct SKYRAGAME_API FSkyraSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "SkyraAudioSettings"))
class SKYRAGAME_API USkyraAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FSkyraSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FSkyraSubmixEffectChainMap> LDRAudioSubmixEffectChain;
