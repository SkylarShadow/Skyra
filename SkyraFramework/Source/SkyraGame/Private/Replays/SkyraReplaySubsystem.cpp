...
#include "SkyraReplaySubsystem.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraReplaySubsystem)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Platform_Trait_ReplaySupport, "Platform.Trait.ReplaySupport");

USkyraReplaySubsystem::USkyraReplaySubsystem()
{
}

bool USkyraReplaySubsystem::DoesPlatformSupportReplays()
{
...
FGameplayTag USkyraReplaySubsystem::GetPlatformSupportTraitTag()
{
	return TAG_Platform_Trait_ReplaySupport.GetTag();
}

void USkyraReplaySubsystem::PlayReplay(USkyraReplayListEntry* Replay)
{
	if (Replay != nullptr)
	{
...
void USkyraReplaySubsystem::RecordClientReplay(APlayerController* PlayerController)
{
	if (ensure(DoesPlatformSupportReplays() && PlayerController))
	{
		FText FriendlyNameText = FText::Format(NSLOCTEXT("Skyra", "SkyraReplayName_Format", "Client Replay {0}"), FText::AsDateTime(FDateTime::UtcNow(), EDateTimeStyle::Short, EDateTimeStyle::Short));
		GetGameInstance()->StartRecordingReplay(FString(), FriendlyNameText.ToString());

		if (USkyraLocalPlayer* SkyraLocalPlayer = Cast<USkyraLocalPlayer>(PlayerController->GetLocalPlayer()))
		{
			// Start a cleanup of existing saved streams
			int32 NumToKeep = SkyraLocalPlayer->GetLocalSettings()->GetNumberOfReplaysToKeep();
			CleanupLocalReplays(SkyraLocalPlayer, NumToKeep);
		}
	}
}

void USkyraReplaySubsystem::CleanupLocalReplays(ULocalPlayer* LocalPlayer, int32 NumReplaysToKeep)
{
...
void USkyraReplaySubsystem::OnEnumerateStreamsCompleteForDelete(const FEnumerateStreamsResult& Result)
{
	...
	if (StreamsToDelete.Num() > DeletingReplaysNumberToKeep)
	{
		...
		UE_LOG(LogSkyra, Log, TEXT("SkyraReplaySubsystem asked to delete replay %s"), *ReplayName);
...
void USkyraReplaySubsystem::OnDeleteReplay(const FDeleteFinishedStreamResult& DeleteResult)
{
	if (!CurrentReplayStreamer.IsValid() || !IsValid(LocalPlayerDeletingReplays))
	{
		// Lost context, don't do anything
	...
	else
	{
		...
		UE_LOG(LogSkyra, Warning, TEXT("Failed to delete replay with error %d!"), (int32)DeleteResult.Result);
...
void USkyraReplaySubsystem::SeekInActiveReplay(float TimeInSeconds)
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float USkyraReplaySubsystem::GetReplayLengthInSeconds() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float USkyraReplaySubsystem::GetReplayCurrentTime() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
UDemoNetDriver* USkyraReplaySubsystem::GetDemoDriver() const
{
