...
void UAsyncAction_QueryReplays::Activate()
{
	...
	ResultList = NewObject<USkyraReplayList>();
...
void UAsyncAction_QueryReplays::OnEnumerateStreamsComplete(const FEnumerateStreamsResult& Result)
{
	...
	{
		USkyraReplayListEntry* NewReplayEntry = NewObject<USkyraReplayListEntry>(ResultList);
		NewReplayEntry->StreamInfo = StreamInfo;
		ResultList->Results.Add(NewReplayEntry);
	}

	// Sort demo names by date
	Algo::SortBy(ResultList->Results, [](const TObjectPtr<USkyraReplayListEntry>& Data) { return Data->StreamInfo.Timestamp.GetTicks(); }, TGreater<>());
