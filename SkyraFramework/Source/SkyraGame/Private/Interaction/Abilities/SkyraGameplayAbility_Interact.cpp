...
#include "SkyraGameplayAbility_Interact.h"
...
#include "UI/IndicatorSystem/SkyraIndicatorManagerComponent.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraGameplayAbility_Interact)
...
USkyraGameplayAbility_Interact::USkyraGameplayAbility_Interact(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	ActivationPolicy = ESkyraAbilityActivationPolicy::OnSpawn;
	InstancingPolicy = EGameplayAbilityInstancingPolicy::InstancedPerActor;
	NetExecutionPolicy = EGameplayAbilityNetExecutionPolicy::LocalPredicted;
}

void USkyraGameplayAbility_Interact::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void USkyraGameplayAbility_Interact::UpdateInteractions(const TArray<FInteractionOption>& InteractiveOptions)
{
	if (ASkyraPlayerController* PC = GetSkyraPlayerControllerFromActorInfo())
	{
		if (USkyraIndicatorManagerComponent* IndicatorManager = USkyraIndicatorManagerComponent::GetComponent(PC))
		{
...
void USkyraGameplayAbility_Interact::TriggerInteraction()
{
