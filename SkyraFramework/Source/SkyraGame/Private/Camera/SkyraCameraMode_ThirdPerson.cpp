...
#include "SkyraCameraMode_ThirdPerson.h"
#include "Camera/SkyraCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/SkyraPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "SkyraCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraCameraMode_ThirdPerson)

namespace SkyraCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

USkyraCameraMode_ThirdPerson::USkyraCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FSkyraPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FSkyraPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FSkyraPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FSkyraPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FSkyraPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FSkyraPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FSkyraPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void USkyraCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void USkyraCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void USkyraCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void USkyraCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	ISkyraCameraAssistInterface* TargetControllerAssist = Cast<ISkyraCameraAssistInterface>(TargetController);

	ISkyraCameraAssistInterface* TargetActorAssist = Cast<ISkyraCameraAssistInterface>(TargetActor);
	...
	ISkyraCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<ISkyraCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		ISkyraCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (ISkyraCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void USkyraCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO ISkyraCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FSkyraPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(SkyraCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void USkyraCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void USkyraCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
