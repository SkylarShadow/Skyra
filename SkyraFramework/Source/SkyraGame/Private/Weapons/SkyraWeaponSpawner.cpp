...
#include "SkyraWeaponSpawner.h"

#include "AbilitySystemBlueprintLibrary.h"
#include "Components/CapsuleComponent.h"
#include "Components/StaticMeshComponent.h"
#include "Engine/World.h"
#include "Equipment/SkyraPickupDefinition.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraWeaponSpawner)
...
// Sets default values
ASkyraWeaponSpawner::ASkyraWeaponSpawner()
{
	...
	CollisionVolume->OnComponentBeginOverlap.AddDynamic(this, &ASkyraWeaponSpawner::OnOverlapBegin);
...
void ASkyraWeaponSpawner::BeginPlay()
{
	Super::BeginPlay();
	...
	else if (const UWorld* World = GetWorld())
	{
		...
		{
			UE_LOG(LogSkyra, Error, TEXT("'%s' does not have a valid weapon definition! Make sure to set this data on the instance!"), *GetNameSafe(this));	
		}
	}
}

void ASkyraWeaponSpawner::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void ASkyraWeaponSpawner::Tick(float DeltaTime)
{
...
void ASkyraWeaponSpawner::OnConstruction(const FTransform& Transform)
{
...
void ASkyraWeaponSpawner::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
...
void ASkyraWeaponSpawner::CheckForExistingOverlaps()
{
...
void ASkyraWeaponSpawner::AttemptPickUpWeapon_Implementation(APawn* Pawn)
{
	if (GetLocalRole() == ROLE_Authority && bIsWeaponAvailable && UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Pawn))
	{
		TSubclassOf<USkyraInventoryItemDefinition> WeaponItemDefinition = WeaponDefinition ? WeaponDefinition->InventoryItemDefinition : nullptr;
...
void ASkyraWeaponSpawner::StartCoolDown()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(CoolDownTimerHandle, this, &ASkyraWeaponSpawner::OnCoolDownTimerComplete, CoolDownTime);
	}
}

void ASkyraWeaponSpawner::ResetCoolDown()
{
	UWorld* World = GetWorld();
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		if (World)
		{
			World->GetTimerManager().SetTimer(CheckOverlapsDelayTimerHandle, this, &ASkyraWeaponSpawner::CheckForExistingOverlaps, CheckExistingOverlapDelay);
		}
	}
...
void ASkyraWeaponSpawner::OnCoolDownTimerComplete()
{
	ResetCoolDown();
}

void ASkyraWeaponSpawner::SetWeaponPickupVisibility(bool bShouldBeVisible)
{
	WeaponMesh->SetVisibility(bShouldBeVisible, true);
}

void ASkyraWeaponSpawner::PlayPickupEffects_Implementation()
{
...
void ASkyraWeaponSpawner::PlayRespawnEffects_Implementation()
{
	if (WeaponDefinition != nullptr)
	{
...
void ASkyraWeaponSpawner::OnRep_WeaponAvailability()
{
...
void ASkyraWeaponSpawner::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(ASkyraWeaponSpawner, bIsWeaponAvailable);
}

int32 ASkyraWeaponSpawner::GetDefaultStatFromItemDef(const TSubclassOf<USkyraInventoryItemDefinition> WeaponItemClass, FGameplayTag StatTag)
{
	if (WeaponItemClass != nullptr)
	{
		if (USkyraInventoryItemDefinition* WeaponItemCDO = WeaponItemClass->GetDefaultObject<USkyraInventoryItemDefinition>())
		{
