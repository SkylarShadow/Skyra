...
#include "SkyraDamageLogDebuggerComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraDamageLogDebuggerComponent)

UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Skyra_Damage_Message);

USkyraDamageLogDebuggerComponent::USkyraDamageLogDebuggerComponent(const FObjectInitializer& ObjectInitializer)
...
void USkyraDamageLogDebuggerComponent::BeginPlay()
{
	Super::BeginPlay();

	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
	ListenerHandle = MessageSubsystem.RegisterListener(TAG_Skyra_Damage_Message, this, &ThisClass::OnDamageMessage);
}

void USkyraDamageLogDebuggerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
...
void USkyraDamageLogDebuggerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if ((TimeSinceDamage >= SecondsBetweenDamageBeforeLogging) && (DamageLog.Num() > 0))
	{
		...
		UE_LOG(LogSkyra, Warning, TEXT("%d impacts in %d distinct frames over %.2f seconds did %.2f damage"),
			NumImpacts, NumFrames, TotalInterval, TotalDamage);
		if (TotalInterval > 0.0)
		{
			UE_LOG(LogSkyra, Warning, TEXT("Interval ranged from %.1f ms to %.1f ms (avg %.1f ms)"),
				MinInterval * 1000.0, MaxInterval * 1000.0, (MaxInterval + MinInterval) / 2.0 * 1000.0);
			UE_LOG(LogSkyra, Warning, TEXT("DPS %.2f"), TotalDamage / TotalInterval);
		}
		UE_LOG(LogSkyra, Warning, TEXT("\n"));
	}
}

void USkyraDamageLogDebuggerComponent::OnDamageMessage(FGameplayTag Channel, const FSkyraVerbMessage& Payload)
{
