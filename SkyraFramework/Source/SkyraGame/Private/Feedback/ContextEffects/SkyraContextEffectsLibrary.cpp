...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraContextEffectsLibrary)


void USkyraContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void USkyraContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState USkyraContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void USkyraContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FSkyraContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<USkyraActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FSkyraContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			USkyraActiveContextEffects* NewActiveContextEffects = NewObject<USkyraActiveContextEffects>(this);
	...
	this->SkyraContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void USkyraContextEffectsLibrary::SkyraContextEffectLibraryLoadingComplete(
	TArray<USkyraActiveContextEffects*> SkyraActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(SkyraActiveContextEffects);
}
