...
#include "SkyraContextEffectComponent.h"

#include "Engine/World.h"
#include "SkyraContextEffectsSubsystem.h"
#include "PhysicalMaterials/PhysicalMaterial.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraContextEffectComponent)
...
USkyraContextEffectComponent::USkyraContextEffectComponent()
{
...
void USkyraContextEffectComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	{
		if (USkyraContextEffectsSubsystem* SkyraContextEffectsSubsystem = World->GetSubsystem<USkyraContextEffectsSubsystem>())
		{
			SkyraContextEffectsSubsystem->LoadAndAddContextEffectsLibraries(GetOwner(), CurrentContextEffectsLibraries);
		}
	}
}

void USkyraContextEffectComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (const UWorld* World = GetWorld())
	{
		if (USkyraContextEffectsSubsystem* SkyraContextEffectsSubsystem = World->GetSubsystem<USkyraContextEffectsSubsystem>())
		{
			SkyraContextEffectsSubsystem->UnloadAndRemoveContextEffectsLibraries(GetOwner());
		}
	}
...
void USkyraContextEffectComponent::AnimMotionEffect_Implementation(const FName Bone, const FGameplayTag MotionEffect, USceneComponent* StaticMeshComponent,
	...
	FVector VFXScale, float AudioVolume, float AudioPitch)
{
	...
	if (const UWorld* World = GetWorld())
	{
		...
		if (USkyraContextEffectsSubsystem* SkyraContextEffectsSubsystem = World->GetSubsystem<USkyraContextEffectsSubsystem>())
		{
			...
			// Spawn effects
			SkyraContextEffectsSubsystem->SpawnContextEffects(GetOwner(), StaticMeshComponent, Bone, 
...
void USkyraContextEffectComponent::UpdateEffectContexts(FGameplayTagContainer NewEffectContexts)
{
	// Reset and update
...
void USkyraContextEffectComponent::UpdateLibraries(
	TSet<TSoftObjectPtr<USkyraContextEffectsLibrary>> NewContextEffectsLibraries)
{
