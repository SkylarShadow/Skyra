...
#include "SkyraTabListWidgetBase.h"

#include "CommonAnimatedSwitcher.h"
#include "CommonButtonBase.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraTabListWidgetBase)

void USkyraTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void USkyraTabListWidgetBase::NativeConstruct()
{
...
void USkyraTabListWidgetBase::NativeDestruct()
{
	for (FSkyraTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool USkyraTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FSkyraTabDescriptor& OutTabInfo)
{
	const FSkyraTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FSkyraTabDescriptor& TabInfo) -> bool
	{
...
void USkyraTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FSkyraTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool USkyraTabListWidgetBase::RegisterDynamicTab(const FSkyraTabDescriptor& TabDescriptor)
{
...
void USkyraTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FSkyraTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void USkyraTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void USkyraTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FSkyraTabDescriptor* TabInfoPtr = nullptr;
	
	FSkyraTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(USkyraTabButtonInterface::StaticClass()))
	{
		...
		{
			ISkyraTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool USkyraTabListWidgetBase::IsFirstTabActive() const
{
...
bool USkyraTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool USkyraTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 USkyraTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void USkyraTabListWidgetBase::SetupTabs()
{
	for (FSkyraTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
