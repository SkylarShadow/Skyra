...
#include "SkyraFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraFrontendStateComponent)
...
USkyraFrontendStateComponent::USkyraFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void USkyraFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void USkyraFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool USkyraFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void USkyraFrontendStateComponent::OnExperienceLoaded(const USkyraExperienceDefinition* Experience)
{
...
void USkyraFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void USkyraFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &USkyraFrontendStateComponent::OnUserInitialized);
...
void USkyraFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &USkyraFrontendStateComponent::OnUserInitialized);
...
void USkyraFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void USkyraFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
