...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraGameInstance)

namespace Skyra
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarSkyraTestEncryption(
		TEXT("Skyra.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarSkyraUseDTLSEncryption(
		TEXT("Skyra.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using Skyra.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarSkyraTestDTLSFingerprint(
		TEXT("Skyra.TestDTLSFingerprint"),
...
USkyraGameInstance::USkyraGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void USkyraGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(SkyraGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(SkyraGameplayTags::InitState_DataAvailable, false, SkyraGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(SkyraGameplayTags::InitState_DataInitialized, false, SkyraGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(SkyraGameplayTags::InitState_GameplayReady, false, SkyraGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &USkyraGameInstance::OnPreClientTravelToSession);
	}
}

void USkyraGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
ASkyraPlayerController* USkyraGameInstance::GetPrimaryPlayerController() const
{
	return Cast<ASkyraPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool USkyraGameInstance::CanJoinRequestedSession() const
{
...
void USkyraGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		USkyraLocalPlayer* LocalPlayer = Cast<USkyraLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void USkyraGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Skyra::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Skyra::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("SkyraTest.pem");
...
void USkyraGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void USkyraGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (Skyra::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (Skyra::bUseDTLSEncryption)
		{
