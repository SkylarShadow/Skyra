...
#include "SkyraAudioSettings.h"
#include "Settings/SkyraSettingsLocal.h"
#include "Sound/SoundEffectSubmix.h"
#include "SoundControlBus.h"
#include "SoundControlBusMix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraAudioMixEffectsSubsystem)

class FSubsystemCollectionBase;

void USkyraAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void USkyraAudioMixEffectsSubsystem::Deinitialize()
{
...
bool USkyraAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void USkyraAudioMixEffectsSubsystem::PostInitialize()
{
	if (const USkyraAudioSettings* SkyraAudioSettings = GetDefault<USkyraAudioSettings>())
	{
		if (UObject* ObjPath = SkyraAudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from Skyra Audio Settings."));
			}
		}

		if (UObject* ObjPath = SkyraAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from Skyra Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = SkyraAudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from Skyra Audio Settings."));
			}
		}
		...
		for (const FSkyraSubmixEffectChainMap& SoftSubmixEffectChain : SkyraAudioSettings->HDRAudioSubmixEffectChain)
		{
			FSkyraAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FSkyraSubmixEffectChainMap& SoftSubmixEffectChain : SkyraAudioSettings->LDRAudioSubmixEffectChain)
		{
			FSkyraAudioSubmixEffectsChain NewEffectChain;
...
void USkyraAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const USkyraSettingsLocal* SkyraSettingsLocal = GetDefault<USkyraSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, SkyraSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, SkyraSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, SkyraSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, SkyraSettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, SkyraSettingsLocal->GetVoiceChatVolume());
			...
			ApplyDynamicRangeEffectsChains(SkyraSettingsLocal->IsHDRAudioModeEnabled());
		}
	}
}

void USkyraAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FSkyraAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FSkyraAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FSkyraAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FSkyraAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void USkyraAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void USkyraAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool USkyraAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
