...
#include "Teams/SkyraTeamStatics.h"
...
UAsyncAction_ObserveTeamColors* UAsyncAction_ObserveTeamColors::ObserveTeamColors(UObject* TeamAgent)
{
	...
	if (TeamAgent != nullptr)
	{
		...
		Action->TeamInterfacePtr = TWeakInterfacePtr<ISkyraTeamAgentInterface>(TeamAgent);
...
void UAsyncAction_ObserveTeamColors::SetReadyToDestroy()
{
	...
	if (ISkyraTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
...
void UAsyncAction_ObserveTeamColors::Activate()
{
	...
	USkyraTeamDisplayAsset* CurrentDisplayAsset = nullptr;
	...
	if (ISkyraTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
		if (UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull))
		{
			...
			CurrentDisplayAsset = USkyraTeamStatics::GetTeamDisplayAsset(World, CurrentTeamIndex);
...
void UAsyncAction_ObserveTeamColors::BroadcastChange(int32 NewTeam, const USkyraTeamDisplayAsset* DisplayAsset)
{
	UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull);
	USkyraTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<USkyraTeamSubsystem>(World);
...
{
	USkyraTeamDisplayAsset* DisplayAsset = USkyraTeamStatics::GetTeamDisplayAsset(TeamAgent, NewTeam);
	BroadcastChange(NewTeam, DisplayAsset);
}

void UAsyncAction_ObserveTeamColors::OnDisplayAssetChanged(const USkyraTeamDisplayAsset* DisplayAsset)
{
