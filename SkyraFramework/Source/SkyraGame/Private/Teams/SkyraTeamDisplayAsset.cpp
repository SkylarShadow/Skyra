...
#include "SkyraTeamDisplayAsset.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraTeamDisplayAsset)

void USkyraTeamDisplayAsset::ApplyToMaterial(UMaterialInstanceDynamic* Material)
{
	if (Material)
	{
...
void USkyraTeamDisplayAsset::ApplyToMeshComponent(UMeshComponent* MeshComponent)
{
	if (MeshComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void USkyraTeamDisplayAsset::ApplyToNiagaraComponent(UNiagaraComponent* NiagaraComponent)
{
	if (NiagaraComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void USkyraTeamDisplayAsset::ApplyToActor(AActor* TargetActor, bool bIncludeChildActors)
{
...
#if WITH_EDITOR
void USkyraTeamDisplayAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	for (USkyraTeamSubsystem* TeamSubsystem : TObjectRange<USkyraTeamSubsystem>())
	{
