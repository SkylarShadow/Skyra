...
#include "CustomSettings/SkyraSettingValueDiscrete_Language.h"
...
UGameSettingCollection* USkyraGameSettingRegistry::InitializeGameplaySettings(USkyraLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			USkyraSettingValueDiscrete_Language* Setting = NewObject<USkyraSettingValueDiscrete_Language>();
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<USkyraSettingsLocal>()->ShouldAutoRecordReplays());

			Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
			Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(USkyraReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<USkyraSettingsLocal>()->GetNumberOfReplaysToKeep());
