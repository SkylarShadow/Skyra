...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Skyra.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Skyra.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Skyra.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Skyra.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Skyra.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Skyra.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Skyra.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Skyra.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Skyra.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Skyra.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FSkyraScalabilitySnapshot::FSkyraScalabilitySnapshot()
{
...
namespace SkyraSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FSkyraScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
		...
		Mode = FSkyraScalabilitySnapshot();
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = USkyraSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : USkyraSettingsLocal::GetDefaultMobileFrameRate();
	}
...
void USkyraSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ESkyraFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	SkyraSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void USkyraSettingsLocal::ResetToCurrentSettings()
{
...
void USkyraSettingsLocal::BeginDestroy()
{
...
USkyraSettingsLocal* USkyraSettingsLocal::Get()
{
	return GEngine ? CastChecked<USkyraSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void USkyraSettingsLocal::ConfirmVideoMode()
{
...
float USkyraSettingsLocal::GetEffectiveFrameRateLimit()
{
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == ESkyraFramePacingMode::DesktopStyle)
	{
...
int32 USkyraSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return SkyraSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void USkyraSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FSkyraScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void USkyraSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void USkyraSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void USkyraSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void USkyraSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void USkyraSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool USkyraSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
ESkyraStatDisplayMode USkyraSettingsLocal::GetPerfStatDisplayState(ESkyraDisplayablePerformanceStat Stat) const
{
	if (const ESkyraStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return ESkyraStatDisplayMode::Hidden;
	}
}

void USkyraSettingsLocal::SetPerfStatDisplayState(ESkyraDisplayablePerformanceStat Stat, ESkyraStatDisplayMode DisplayMode)
{
	if (DisplayMode == ESkyraStatDisplayMode::Hidden)
	{
...
void USkyraSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void USkyraSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float USkyraSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void USkyraSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float USkyraSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void USkyraSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float USkyraSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void USkyraSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float USkyraSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void USkyraSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void USkyraSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 USkyraSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 USkyraSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool USkyraSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 USkyraSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return SkyraSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 USkyraSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return SkyraSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void USkyraSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 USkyraSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == ESkyraFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return SkyraSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void USkyraSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ESkyraFramePacingMode::MobileStyle)
	{
...
void USkyraSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void USkyraSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ESkyraFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = SkyraSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void USkyraSettingsLocal::ClampMobileQuality()
{
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ESkyraFramePacingMode::MobileStyle)
	{
...
void USkyraSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = SkyraSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void USkyraSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = SkyraSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = SkyraSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString USkyraSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void USkyraSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void USkyraSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool USkyraSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool USkyraSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = SkyraSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool USkyraSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void USkyraSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (USkyraAudioMixEffectsSubsystem* SkyraAudioMixEffectsSubsystem = World->GetSubsystem<USkyraAudioMixEffectsSubsystem>())
			{
				SkyraAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool USkyraSettingsLocal::CanRunAutoBenchmark() const
{
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool USkyraSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void USkyraSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void USkyraSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float USkyraSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void USkyraSettingsLocal::SetOverallVolume(float InVolume)
{
...
float USkyraSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void USkyraSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float USkyraSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void USkyraSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float USkyraSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void USkyraSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float USkyraSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void USkyraSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void USkyraSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void USkyraSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void USkyraSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void USkyraSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 USkyraSettingsLocal::GetOverallScalabilityLevel() const
{
...
void USkyraSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == ESkyraFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = SkyraSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void USkyraSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName USkyraSettingsLocal::GetControllerPlatform() const
{
	...
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const USkyraAudioSettings* SkyraAudioSettings = GetDefault<USkyraAudioSettings>())
			{
				...
				if (UObject* ObjPath = SkyraAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from Skyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = SkyraAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from Skyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = SkyraAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from Skyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = SkyraAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from Skyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = SkyraAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from Skyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = SkyraAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Skyra Audio Settings."));
					}
				}
			}
		}
...
void USkyraSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void USkyraSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const USkyraPlatformSpecificRenderingSettings* PlatformSettings = USkyraPlatformSpecificRenderingSettings::Get();
	const TArray<FSkyraQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FSkyraQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const USkyraPlatformEmulationSettings* Settings = GetDefault<USkyraPlatformEmulationSettings>();
	...
	{
	case ESkyraFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case ESkyraFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case ESkyraFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void USkyraSettingsLocal::UpdateConsoleFramePacing()
{
...
void USkyraSettingsLocal::UpdateDesktopFramePacing()
{
...
void USkyraSettingsLocal::UpdateMobileFramePacing()
{
...
void USkyraSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
