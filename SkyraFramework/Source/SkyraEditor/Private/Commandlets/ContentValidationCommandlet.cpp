...
DEFINE_LOG_CATEGORY_STATIC(LogSkyraContentValidation, Log, Log);
...
{
	UE_LOG(LogSkyraContentValidation, Display, TEXT("Running ContentValidationCommandlet commandlet..."));
	...
	if (P4FilterString && !P4FilterString->IsEmpty())
	{
		...
		{
			UE_LOG(LogSkyraContentValidation, Display, TEXT("ContentValidation returning 1. Failed to get changed files."));
			ReturnVal = 1;
		}
	}
	...
	if (bP4Opened)
	{
		...
		{
			UE_LOG(LogSkyraContentValidation, Error, TEXT("P4 workspace was not found when using P4Opened"));
			UE_LOG(LogSkyraContentValidation, Display, TEXT("ContentValidation returning 1. Workspace not found."));
			ReturnVal = 1;
		}
	}
...
bool UContentValidationCommandlet::GetAllChangedFiles(IAssetRegistry& AssetRegistry, const FString& P4CmdString, TArray<FString>& OutChangedPackageNames, TArray<FString>& DeletedPackageNames, TArray<FString>& OutChangedCode, TArray<FString>& OutChangedOtherFiles) const
{
	...
	if (LaunchP4(P4CmdString, Results, ReturnCode))
	{
				...
				if (Result.Split(TEXT("#"), &DepotPathName, &ExtraInfoAfterPound))
				{
					if (DepotPathName.EndsWith(TEXT(".uasset")) || DepotPathName.EndsWith(TEXT(".umap")))
					{
						FString FullPackageName;
						{
							...
							if (DepotPathName.Split(TEXT("SkyraGame/Content/"), nullptr, &PostContentPath)) //@TODO: RENAME: Potential issue when modules are renamed
							{
						...
						if (FullPackageName.IsEmpty())
						{
							...
							if (DepotPathName.Split(TEXT("SkyraGame/Plugins/"), nullptr, &PostPluginsPath))
							{
					...
					{
						FString PostSkyraGamePath;
						if (DepotPathName.Split(TEXT("/SkyraGame/"), nullptr, &PostSkyraGamePath))
						{
							if (DepotPathName.EndsWith(TEXT(".cpp")))
							{
								OutChangedCode.Add(PostSkyraGamePath);
							}
							...
							{
								OutChangedOtherFiles.Add(PostSkyraGamePath);
							}
						}
					}
				}
		...
		{
			UE_LOG(LogSkyraContentValidation, Error, TEXT("p4 returned non-zero return code %d"), ReturnCode);
		}
	}
...
void UContentValidationCommandlet::GetAllPackagesOfType(const FString& OfTypeString, TArray<FString>& OutPackageNames) const
{
	...
	for (const FString& Type : Types)
	{
		...
		{
			UE_LOG(LogSkyraContentValidation, Error, TEXT("Failed to convert short class name \"%s\" to path name. Please use class path names."), *Type);
		}
		else
		{
...
bool UContentValidationCommandlet::LaunchP4(const FString& Args, TArray<FString>& Output, int32& OutReturnCode) const
{
	...
	{
		UE_LOG(LogSkyraContentValidation, Error, TEXT("Failed to launch p4."));
	}
...
FString UContentValidationCommandlet::GetLocalPathFromDepotPath(const FString& DepotPathName) const
{
	...
	if (!Workspace.IsEmpty())
	{
		...
		if (LaunchP4(P4WhereCommand, WhereResults, ReturnCode))
		{
			...
			{
				UE_LOG(LogSkyraContentValidation, Warning, TEXT("GetAllChangedFiles failed to run p4 'where'. WhereResults[0] = '%s'. Not adding any validation for %s"), WhereResults.Num() > 0 ? *WhereResults[0] : TEXT("Invalid"), *DepotPathName);
			}
		}
	}
