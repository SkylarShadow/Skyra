...
#include "Utilities/TexImport/TextureImportPipeline.h"
...
#include "Utilities/TexImport/InterchangeType.h"
...
DECLARE_LOG_CATEGORY_EXTERN(MyLogInterchange, Log, All);
...
void UTextureImportPipeline::FilterPropertiesFromTranslatedData(UInterchangeBaseNodeContainer* InBaseNodeContainer)
{
...
void UTextureImportPipeline::ExecutePipeline(UInterchangeBaseNodeContainer* InBaseNodeContainer,
                                         const TArray<UInterchangeSourceData*>& InSourceDatas,
                                         const FString& ContentBasePath)
{
	if (!InBaseNodeContainer)
	{
		UE_LOG(MyLogInterchange, Warning,
		       TEXT(" UTextureImportPipeline : Cannot execute pre-import pipeline because InBaseNodeContrainer is null"));
		return;
	}
	UE_LOG(MyLogInterchange, Log, TEXT("ContentBasePath: %s"), *ContentBasePath);
	...
	if (bImportTextures)
	{
		...
		{
			TextureFactoryNode = HandleCreationOfTextureFactoryNode(TextureNode,ContentBasePath);
		}
		...
		const bool bOverrideAssetName = TextureNodes.Num() == 1 && IsStandAlonePipeline() && !OverrideAssetName.
			IsEmpty();
...
void UTextureImportPipeline::ExecutePostFactoryPipeline(const UInterchangeBaseNodeContainer* InBaseNodeContainer,
...
TextureAddress ConvertWrap(EInterchangeTextureWrapMode WrapMode)
{
	switch (WrapMode)
	{
...
UInterchangeTextureFactoryNode* UTextureImportPipeline::HandleCreationOfTextureFactoryNode(
	const UInterchangeTextureNode* TextureNode,const FString& ContentPath)
{
	UClass* FactoryClass = GetDefaultFactoryClassFromTextureNodeClass(TextureNode->GetClass());
	...
	if (FactoryClass == UInterchangeTexture2DFactoryNode::StaticClass() && InterchangeTextureFactoryNode)
	{
		...
		UInterchangeTexture2DFactoryNode* Texture2DFactoryNode = static_cast<UInterchangeTexture2DFactoryNode*>(
		...
		if (const UInterchangeTexture2DNode* Texture2DNode = Cast<UInterchangeTexture2DNode>(TextureNode))
		{
			...
			{
				Texture2DFactoryNode->SetCustomAddressX(ConvertWrap(WrapU));
			}

			EInterchangeTextureWrapMode WrapV;
			if (Texture2DNode->GetCustomWrapV(WrapV))
			{
				Texture2DFactoryNode->SetCustomAddressY(ConvertWrap(WrapV));
			}

			bool HasPath= false;
			for (auto InterchangeTex : InterchangeTextures)
			{
				if(ContentPath.Contains(InterchangeTex.Path))
				{
					HasPath = true;
					InterchangeTextureFactoryNode->SetCustomLODGroup(InterchangeTex.LODGroup);
					break;
				}
			}
			if(HasPath)
			{
				UE_LOG(MyLogInterchange,Log,TEXT("%s Set TextureGroup: "), *ContentPath);
			}
		}
		...
		if (SourceBlocks.IsEmpty() && bImportUDIMs && SourceFile)
		{
			...
			SourceBlocks = UE::TextureUtilitiesCommon::GetUDIMBlocksFromSourceFile(
				SourceFile.GetValue(), UE::TextureUtilitiesCommon::DefaultUdimRegexPattern, &PrettyAssetName);
...
UInterchangeTextureFactoryNode* UTextureImportPipeline::CreateTextureFactoryNode(
	const UInterchangeTextureNode* TextureNode, const TSubclassOf<UInterchangeTextureFactoryNode>& FactorySubclass)
{
	...
	static_assert(
		FCommonTextureFilterModes(EInterchangeTextureFilterMode::Nearest) ==
		FCommonTextureFilterModes(TextureFilter::TF_Nearest),
		"EInterchangeTextureFilterMode::Nearest differs from TextureFilter::TF_Nearest");
	static_assert(
		FCommonTextureFilterModes(EInterchangeTextureFilterMode::Bilinear) ==
		FCommonTextureFilterModes(TextureFilter::TF_Bilinear),
		"EInterchangeTextureFilterMode::Bilinear differs from TextureFilter::TF_Bilinear");
	static_assert(
		FCommonTextureFilterModes(EInterchangeTextureFilterMode::Trilinear) ==
		FCommonTextureFilterModes(TextureFilter::TF_Trilinear),
		"EInterchangeTextureFilterMode::Trilinear differs from TextureFilter::TF_Trilinear");
	static_assert(
		FCommonTextureFilterModes(EInterchangeTextureFilterMode::Default) ==
		FCommonTextureFilterModes(TextureFilter::TF_Default),
		"EInterchangeTextureFilterMode::Default differs from TextureFilter::TF_Default");
...
#if WITH_EDITOR
void AdjustTextureForNormalMap(UTexture* Texture, const FImageView& MipToAnalyze, bool bFlipNormalMapGreenChannel)
{
	if (Texture)
	{
		if (UE::NormalMapIdentification::HandleAssetPostImport(Texture, MipToAnalyze))
		{
			UE_LOG(MyLogInterchange, Display, TEXT("Auto-detected normal map"));
...
void UTextureImportPipeline::PostImportTextureAssetImport(UObject* CreatedAsset, bool bIsAReimport)
{
#if WITH_EDITOR
	...
	bool HasPath= false;
	FTextureSourceColorSettings TargetColorSettings;
	...
	UE_LOG(LogTemp,Log,TEXT("GetPathName: %s"),*Texture->GetPathName());
	for (auto InterchangeTex : InterchangeTextures)
	{
		if (Texture->GetPathName().Contains( InterchangeTex.Path))
		{
			Texture->SourceColorSettings=InterchangeTex.SourceColorSettings;
			bRunNormapMapDetection=false;
			return;;
		}
	}
	...
	{
		UE_LOG(MyLogInterchange, Error,
		       TEXT(
			       "PostImport Texture failed to lock mip data, actions (like normal map detection) not performed on %s"
		       ), *Texture->GetPathName());
		return;
	}
	...
	if (bRunNormapMapDetection)
	{
		...
		AdjustTextureForNormalMap(Texture, LockedMip0.Image, bFlipNormalMapGreenChannel);
	}
