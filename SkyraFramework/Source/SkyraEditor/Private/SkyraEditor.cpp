...
#include "GameModes/SkyraExperienceManager.h"
...
DEFINE_LOG_CATEGORY(LogSkyraEditor);
...
{
	return HasNoPlayWorld() && !GetDefault<USkyraDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<USkyraDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
 ...
 * FSkyraEditorModule
 */
class FSkyraEditorModule : public FDefaultGameModuleImpl
{
	typedef FSkyraEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (Settings && !Settings->GetDisplayPluginFolders())
		{
			Settings->SetDisplayPluginFolders(true);
			Settings->SaveConfig();
		}
#endif
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FSkyraEditorModule::ModulesChangedCallback);
		...
		// Register the Context Effects Library asset type actions.
		{
			...
			TSharedRef<FAssetTypeActions_SkyraContextEffectsLibrary> AssetAction = MakeShared<FAssetTypeActions_SkyraContextEffectsLibrary>();
			SkyraContextEffectsLibraryAssetAction = AssetAction;
	...
	{
		USkyraExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<USkyraExperienceManager>();
	...
	virtual void ShutdownModule() override
	{
		// Unregister the Context Effects Library asset type actions.
		{
			...
			TSharedPtr<IAssetTypeActions> AssetAction = SkyraContextEffectsLibraryAssetAction.Pin();
...
private:
	TWeakPtr<IAssetTypeActions> SkyraContextEffectsLibraryAssetAction;
	FDelegateHandle ToolMenusHandle;
};

IMPLEMENT_MODULE(FSkyraEditorModule, SkyraEditor);
